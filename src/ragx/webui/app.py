...
def main():
    ...
    with col1:
        ...
        cfg.api_key = st.text_input("API Key", value=cfg.api_key)
        ...
        selected_metrics = st.multiselect("Select Evaluation Metrics", options=AVAILABLE_METRICS, default=AVAILABLE_METRICS[:5])

        # Button to Run
        if st.button("Run Evaluation"):
            with st.spinner("Running evaluation..."):
                evaluation_results = run(cfg, selected_metrics)
